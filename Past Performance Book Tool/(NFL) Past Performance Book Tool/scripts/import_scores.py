...
def calculate_averages(team_data_pts, team_data_allowed):
    ...
    return (
        ...
        f"{average_17_allowed:.2f}" if not pd.isna(average_17_allowed) else ""
    )
...
def main():
    ...
    for team in team_names:
        wins_pts = df[df['Winner/tie'] == team]['PtsW']
        wins_allowed = df[df['Winner/tie'] == team]['PtsL']
        losses_pts = df[df['Loser/tie'] == team]['PtsL']
        losses_allowed = df[df['Loser/tie'] == team]['PtsW']
