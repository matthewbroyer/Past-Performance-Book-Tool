...
def calculate_points(data, team1, team2):
    ...
    for row in data:
        if (row["Winner/tie"] == team1 or row["Loser/tie"] == team1) and (row["Winner/tie"] == team2 or row["Loser/tie"] == team2):
            team1_points += float(row["PtsW"])
            team2_points += float(row["PtsL"])
            games_calculated += 1
...
def update_excel_with_averages(CSV_PATH, EXCEL_PATH):
    ...
    # Clear the specified column (24rd column)
