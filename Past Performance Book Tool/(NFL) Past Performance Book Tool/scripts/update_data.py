...
def clean_data(year_data, year):
    ...
    # Remove rows with 2 or more empty cells side by side
    year_data = year_data.dropna(thresh=year_data.shape[1] - 1)
...
# Determine the target years based on the current date
if current_month < 10:
    # If it's before October in the current year, scrape the previous num_years years
    years_to_scrape = [current_year - i for i in range(num_years)]
else:
    # If it's October or later, scrape the current year and the previous (num_years - 1) years
    years_to_scrape = [current_year] + [current_year - i for i in range(1, num_years)]
