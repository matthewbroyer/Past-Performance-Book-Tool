...
def calculate_averages(team_data_pts, team_data_allowed):
    ...
    return (
        ...
        f"{average_82_allowed:.2f}" if not pd.isna(average_82_allowed) else ""
    )
...
def main():
    ...
    for team in team_names:
        wins_pts = df[df['Visitor'] == team]['G']
        wins_allowed = df[df['Visitor'] == team]['G.1']
        losses_pts = df[df['Home'] == team]['G.1']
        losses_allowed = df[df['Home'] == team]['G']
