...
def calculate_points(data, team1, team2):
    ...
    for row in data:
        if (row["Visitor/Neutral"] == team1 or row["Home/Neutral"] == team1) and (row["Visitor/Neutral"] == team2 or row["Home/Neutral"] == team2):
            team1_points += float(row["PTS"])
            team2_points += float(row["PTS.1"])
            games_calculated += 1
